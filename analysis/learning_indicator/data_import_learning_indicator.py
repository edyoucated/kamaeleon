...
from analysis.analysis_helper import ORGANIZATION_IDS_LEARNING_INDICATOR as ORGANIZATION_IDS
...
if __name__ == "__main__":
    ...
    # OLD QUERY
    # query_dict = {
    #     "bri_role_skill": f"select ROLE_ID, brs.SKILL_ID, LEARN_ORDER from BRI_ROLE_SKILL brs left join DIM_SKILL s on brs.SKILL_ID = s.SKILL_ID where s.OWNER_ORGANIZATION_ID in ({ORGANIZATION_ID_FILTER});", 
    #     "skill": f"select SKILL_ID, TITLE_EN, TITLE_DE from DIM_SKILL where OWNER_ORGANIZATION_ID in ({ORGANIZATION_ID_FILTER});",
    #     "learning_path": f"select LEARNING_PATH_ID, TITLE_DE, TITLE_EN, OWNER_ORGANIZATION_ID from DIM_LEARNING_PATH where OWNER_ORGANIZATION_ID in ({ORGANIZATION_ID_FILTER});",
    ...
    #     "goal": f"select * from FCT_GOAL where ORGANIZATION_ID in ({ORGANIZATION_ID_FILTER});",
    #     "goal_activity": f"select * from FCT_GOAL_ACTIVITY where NEW_ORGANIZATION_ID in ({ORGANIZATION_ID_FILTER});",
    #     "periodic_workload": f"select pw.* from FCT_PERIODIC_WORKLOAD pw left join FCT_GOAL g on pw.GOAL_ID = g.GOAL_ID where g.ORGANIZATION_ID in ({ORGANIZATION_ID_FILTER});",
    #     "periodic_workload_trigger": f"select pwt.* from FCT_PERIODIC_WORKLOAD_TRIGGER pwt left join FCT_GOAL g on pwt.GOAL_ID = g.GOAL_ID where g.ORGANIZATION_ID in ({ORGANIZATION_ID_FILTER});",
        
    #     "dim_skill_skill_dependency": f"select dss.* from DIM_SKILL_SKILL_DEPENDENCY dss left join DIM_SKILL ds on dss.PREREQUISITE_SKILL_ID = ds.SKILL_ID where OWNER_ORGANIZATION_ID in ({ORGANIZATION_ID_FILTER});",
    ...
    #     "learning_path_exam": f"select * from FCT_LEARNING_PATH_EXAM where CONTEXT_ORGANIZATION_ID in ({ORGANIZATION_ID_FILTER});",
    #     "user_skill_history": f"select ush.* from FCT_USER_SKILL_HISTORY ush left join DIM_USER u on ush.USER_ID = u.USER_ID where ORGANIZATION_ID_FIRST_JOINED in ({ORGANIZATION_ID_FILTER});",
