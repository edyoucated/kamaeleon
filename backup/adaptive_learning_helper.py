...
def resolve_research_id(x: str): 
    result = None
    x = json.loads(x)
    if x["name"] == "RESEARCH-ID":
        if re.search(r"[a-zA-ZÄÖÜäöü]\d{2}[a-zA-ZÄÖÜäöü]\d", x["value"].strip()):
            result = x["value"].upper() 
    return result


def load_research_id(x: str):
    if not x:
        return None
    x = json.loads(x)
    if x["name"] == "RESEARCH-ID":
...
def load_users(
    ...
    if resolve_rid:
        ...
        membership["research_id"] = membership["reference_data_attribute_1"].apply(lambda x: load_research_id(x) if not pd.isna(x) else None)
